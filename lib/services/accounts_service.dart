...
class AccountsService {
  ...
  static const _dropdownPath = '/accounts/accounts/accountDropdown';
  ...
  Future<AccountStatus> getAccountStatus(String accountId) async {
    final json =
        await _client.get(_statusPath, query: {'accountId': accountId});
    final body = json as Map<String, dynamic>;
    if (body['resultCode'] != 1) {
      throw Exception(
          body['message']?.toString() ?? 'Failed to fetch account status');
    }
